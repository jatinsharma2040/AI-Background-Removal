    ...
    <div className='pb-10 md:py-20 mx-2'>
        ...
        bg-gradient-to-r from-gray-900 to-gray-400 bg-clip-text text-transparent'>
            ...
            <div className='relative w-full max-w-3xl overflow-hidden m-auto rounded-xl'>
                {/* Background image */}
                <img src={assets.image_w_bg} style={{clipPath: `inset(0 ${100.2 - sliderPostition}% 0 0 )`}} />

                {/* Foreground image */}
                <img className='absolute top-0 left-0 w-full h-full' 
                src={assets.image_wo_bg} style={{clipPath: `inset(0 0 0 ${sliderPostition}%)`}} />

                {/* slider */}
                <input className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2
                w-full z-10 slider'
                type='range' min={0} max={100} value={sliderPostition} onChange={handleSliderChange}/>
            </div>
    </div>
  )
}
