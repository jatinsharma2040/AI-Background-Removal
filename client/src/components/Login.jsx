  ...
  return (
    ...
    bg-black/30 flex justify-center items-center'>
        ...
        <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
          ...
          {state !== 'Login' && <div className='border px-6 py-2 flex items-center gap-2 rounded-full
            mt-5'>
          ...
          <button className='bg-black w-full text-white py-2 rounded-full cursor-pointer'>
            {state === 'Login' ? 'Login' : "Create Account"}
          </button>

          {state === 'Login' ? <p className='mt-5 text-center'>Don't have an account?
            <span className='text-black cursor-pointer' onClick={() => setState('Sign Up')}> Sign up</span></p>
            :
            <p className='mt-5 text-center'>Already have an account?
              <span className='text-black cursor-pointer' onClick={() => setState('Login')}> Login</span></p>}
